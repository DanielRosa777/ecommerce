<?php
    ...
    class User extends Model{
        ...
        public static function getForgot($email){
            ...
            $results = $sql->select("SELECT * FROM tb_persons a INNER JOIN tb_users b 
                        USING(idperson) WHERE a.desemail = :email", array(
                            ":email" => $email
            ));

            if(count($results) === 0){
                throw new \Exception("1Não foi possivel recuperar a senha.");
            }else{
                $data = $results[0];
                ...
                if(count($results2) === 0){
                    throw new \Exception("2Não foi possivel recuperar a senha.");
                }else{
                    $dataRecovery = $results2[0];

                    $code = openssl_encrypt($dataRecovery['idrecovery'], 'AES-128-CBC', pack("a16",
                        User::SECRET), 0, pack("a16", User::SECRET_IV));
                    $code = base64_encode($code);
                    $link = "https://www.hcodecommerce.pt/admin/forgot/reset?code=$code";
